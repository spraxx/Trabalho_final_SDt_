...
import com.mkyong.io.howto.GetScripts;
...
public class processManager extends UnicastRemoteObject implements ProcessManagerInterface {
    ...
    GetScripts getS = new GetScripts();
    ...
    public void ProcScript(String script) throws IOException {
        getS.ProcessScript(script);
    }
    ...
    public String ProcRequest(ProcessRequest pRequest) throws IOException {
        ...
        if(usageCPU < 60) {
            while(!waitL.isEmpty()){
                ProcScript(waitL.get(0).getpScript());
                showCommand(pRequest);
                waitL.remove(0);
            }
            showCommand(pRequest);
            ProcScript(pRequest.getpScript());
        }
